r"""100-step funfetti BTRM training with multi-resolution data.

Exercises the full funfetti batching stack:
  - BTRMPairSampler with FLOPS-weighted trajectory selection
  - BinPackScheduler for mixed-resolution FlexAttention batches
  - score_differentiable_packed() for multi-image packed scoring
  - ValidationMetrics multi-indexed covariance tracker
  - TrainingArtifacts with funfetti diagnostic charts (Plots A-E)
...
Execution:
  PYTHONUNBUFFERED=1 /mnt/f/dox/repos/ai/futudiffu/.venv/Scripts/python.exe ^
      F:\dox\repos\ai\futudiffu\scripts_ii\run_funfetti_100step.py
"""
...
def main():
    ...
    print("=" * 70)
    print(f"  FUNFETTI 100-STEP BTRM TRAINING")
    ...
    # ==================================================================
    # Phase 4: Run 100-step funfetti training
    # ==================================================================
    print("\n" + "=" * 60)
    print(f"  Phase 4: Funfetti training ({N_STEPS} steps)")
    print("=" * 60)
    ...
    training_curve = train_btrm_differentiable(
        ...
        force_sdpa=False,  # SageAttention INT8 QK for production memory profile
        output_dir=str(OUTPUT_DIR),
        artifacts=artifacts,
    ...
    # ==================================================================
    # Phase 5: Generate analysis (charts A-E + standard)
    # ==================================================================
    print("\n" + "=" * 60)
    ...
    try:
        ...
        sample_keys = []
        # Pick 4 images from each resolution tier
        for idx in traj_ids:
            for pos in positions:
                if pos.traj_id == idx:
    ...
    print(f"\n{'=' * 70}")
    print(f"  FUNFETTI 100-STEP RUN COMPLETE")
    print(f"{'=' * 70}")
